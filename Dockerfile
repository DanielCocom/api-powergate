...
WORKDIR /src
COPY ["api-powergate.csproj", "."]
RUN dotnet restore "./api-powergate.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./api-powergate.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase se usa para publicar el proyecto de servicio que se copiará en la fase final.
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./api-powergate.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
